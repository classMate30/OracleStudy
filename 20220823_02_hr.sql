...
복합프라이머리 라는 문법이 있지만, 가급적이면 안만들게 하기 위해 2정규화를 하는 거라고 보면됨
ex)원석 , 현하 / 동현, 태민
...
--  『1(일) : many(다)』관계로 깨뜨리는 과정이 바로 제 4정규화 수행 과정이다.
--  → 파생 테이블 생성 → 다:다 관계를 1:다 관계로 깨뜨리는 역할 수행




-- 역정규화(비정규화)        (합치는것..?) (상황에 따라 할때도 있고 안할때도 있음)
...
--  (10 * 30 Byte) + (1000000 * 60) = 300 + 60000000 = 60000300 Byte


--> 『사원』테이블을 역정규화 한 후 이 테이블만 읽어올 때의 크기
--   (즉, 부서 테이블의 부서명 컬럼을 사원 테이블에 추가한 경우)
--  (1000000 * 70) = 70000000 Byte
...
--> 『부서』테이블과 『사원』 테이블을 JOIN 했을 때의 크기
--  (500000 * 30 Byte) + (1000000 * 60) = 15000000 + 60000000 = 75000000 Byte
...
/*
정규화 
데이터 베이스 메모리 낭비를 막기위해 식별자를 갖게 하는 방법으로 테이블을 나누고 분리하는 것

정규화는
제1정규화 제2정규화 제3정규화 제4정규화 역정규화
순으로 기억 할것

제1 정규화는 반복되는 것들을 식별자를 기준으로를 떠올릴땐 
LGLGLG SKSK 와 같은 예제를 떠올릴 것
부모와 자식 관계를 가짐
-참조받는 부모
-참조하는 자식
제 1정규화를 마치고 나면 1대다 관계 형성하게 됨

부모쪽 프라이머리키 제약조건을 가질수 잇음
고유한값 필 (중복X),NOT NULL

프라이머리키 제약조건 한테이블 최대 1개
프라이머리키를 구성하는 컬럼이 1개 - 싱글

프라이머리키를 구성하는 컬럼이 다수 - 복합
...
--   다시 입력 할 수 없도록 처리한다(유일성)
--   UNIQUE INDEX가 오라클 내부적으로 자동으로 생성된다.


-- 3. 형식 및 구조
--    ①컬럼 레벨의 형식
--    컬럼명 데이터타입[CONSTANT,CONSTRAINT명] PRIMARY KEY[(컬럼명,...)]
...
HR	SYS_C007122	P	TBL_TEST1					ENABLED	NOT DEFERRABLE	IMMEDIATE	VALIDATED	GENERATED NAME			2022-08-24	HR	SYS_C007122		
*/
...
HR	SYS_C007122	TBL_TEST1	COL1	1
*/
...
SELECT *
FROM USER_CONSTRAINTS U1 JOIN USER_CONS_COLUMNS U2
ON U1.OWNER = U2.OWNER; 

SELECT U1.OWNER"소유주",U1.CONSTRAINT_NAME"제약조건명",U1.TABLE_NAME"테이블명",U1.CONSTRAINT_TYPE"제약조건종류",COLUMN_NAME"컬럼명"
FROM USER_CONSTRAINTS U1 JOIN USER_CONS_COLUMNS U2
...
HR	SYS_C007122	TBL_TEST1	P	COL1
*/
...
AS
SELECT UC.OWNER"OWNER",UC.CONSTRAINT_NAME"CONSTRAINT_NAME"
    ,   UC.TABLE_NAME"TABLE_NAME",UC.CONSTRAINT_TYPE"CONSTRAINT TYPE"
    ,   UCC.COLUMN_NAME"COLUMN_NAME",UC.SEARCH_CONDITION"SEARCH_CONDITION"
...
--==>>
/*
HR	SYS_C007127	TBL_TEST5	P	COL1		
HR	SYS_C007128	TBL_TEST5	U	COL2		
*/


--데이터 입력
INSERT INTO TBL_TEST5(COL1, COL2) VALUES(1, 'TEST');
INSERT INTO TBL_TEST5(COL1, COL2) VALUES(1, 'TEST');            -->에러 발생(P)
INSERT INTO TBL_TEST5(COL1, COL2) VALUES(2, 'ABCD');
INSERT INTO TBL_TEST5(COL1, COL2) VALUES(3, 'ABCD');            -->에러 발생(U)
INSERT INTO TBL_TEST5(COL1, COL2) VALUES(3, NULL);
INSERT INTO TBL_TEST5(COL1) VALUES(4);
INSERT INTO TBL_TEST5(COL1, COL2) VALUES(5, 'ABCD');            -->에러 발생 (U)
...
INSERT INTO TBL_TEST8(COL1, COL2, COL3) VALUES(1, '민찬우', 100);      --> 에러 발생(PK)
INSERT INTO TBL_TEST8(COL1, COL2, COL3) VALUES(2, '민찬우', 101);      --> 에러 발생(CK)
INSERT INTO TBL_TEST8(COL1, COL2, COL3) VALUES(2, '민찬우', -1);       --> 에러 발생(CK)
...
--==>>
/*
HR	SYS_C007133	TBL_TEST8	C	COL3	COL3 BETWEEN 0 AND 100	
HR	SYS_C007134	TBL_TEST8	P	COL1		
*/
...
INSERT INTO TBL_TEST9(COL1, COL2,COL3) VALUES(1, '민찬우',100); --에러 PRIMARY KEY
INSERT INTO TBL_TEST9(COL1, COL2,COL3) VALUES(2, '민찬우',101); -- 에러 CHECK 범위 초과
INSERT INTO TBL_TEST9(COL1, COL2,COL3) VALUES(2, '민찬우',-1);
...
--○ TBL_TESTMEMBER 테이블의 SSN컬럼(주민등록번호 컬럼)에서
...
--TBL_TESTMEMBER의 SID 컬럼에는 PRIMARY KEY제약조건과 SSN컬럼에는 CHECK제약 조건을 추가
-- SSN컬럼에 추가되는 CHECK제약 조건은 8번째 자리 수가 1,2,3,4만 받을 수 있게 설정
...
ALTER TABLE TBL_TESTMEMBER
ADD ( CONSTRAINT TESTMEMBER_SID_PK PRIMARY KEY(SID)
    , CONSTRAINT TESTMEMBER_SSN_CK CHECK (주민등록번호 8번째 자리 1개가 '1' 또는 '2'또는 '3'또는 '4') );
...
INSERT INTO TBL_TESTMEMBER(SID, NAME, SSN,TEL)
VALUES(5, '박원석', '980301-5234567', '010-5555-5555');                --에러발생

INSERT INTO TBL_TESTMEMBER(SID, NAME, SSN,TEL)
VALUES(6, '한은영', '990729-6234567', '010-6666-6666');                --에러발생
...
--■■■ FOREIGN KEY(FK:F:R)■■■--
-- 1. 참조 키(R) 또는 외래 키(FK:F)는 두 테이블의 데이터간 연결을 설정하고
--    강제 적용시키는데 사용되는 열이다.
...
-- 컬럼명 데이터타입,
-- 컬럼명 데이터타입,
-- CONSTRAINT CONSTRAINT명 FOREIGH KEY(컬럼명)
--            REFERENCES 참조테이블명(참조컬럼명)
--            [ON DELETE CASCADE | ON DELETE SET NULL] → 추가 옵션
...
--    그리고 이 때, 부모 테이블에는 반드시 PK 또는 UK제약조건이
--    설정된 컬럼이 존재해야 한다.
...
/*
컬럼명 CHAR   ← (길이 생략시)한글자만
컬럼명 NUMBER ← (길이 생략시)넘버의 표현범위를 다 가짐
*/
...
--==>>
/*
HR	SYS_C007142	TBL_EMP1	P	SID		
HR	SYS_C007143	TBL_EMP1	R	JIKWI_ID		NO ACTION
*/
-- DELETE_RULE 칸은 추가옵션의 여부를 나타내는것
