...
-- 자바에서 클래스를 위주로 생각했다면
-- 오라클에서 중심으로 다룰 개념은 『테이블』

--○ 테이블 생성(테이블명: TBL_EXAMPLE1)
CREATE TABLE TBL_EXAMPLE1
( NO        NUMBER  
, NAME      VARCHAR2(10)
, ADDR      VARCHAR2(20)            --VARCHAR가변형  // CHAR 고정형 
);
...
--○ TBL_EXAMPLE1과 EXAMPLE2 테이블이
--   각각 어떤 테이블스페이스에 저장되어있는지 조회
SELECT TABLE_NAME, TABLESPACE_NAME
FROM USER_TABLES;
--==>>
/*
...
/*=====================================================================
    ...
    WHERE 조건절            --②+           
    GROUP BY 절             --③+           묶겠다는 것
    HAVING 조건절           --④+           묶여져있을때의 조건(묶여져 있을때 그룹에대한 조건)
    ORDER BY 절             --⑥+           정렬을 수행하게 하는 절
...
2. 문자형 CHAR          → 고정형 크기
          ...
          CHAR(10)      ←← '강의실'       6Byte  이지만 10Byte를 소모 
          CHAR(10)      ←←'실습조영관'   10Byte    
          CHAR(10)      ←←'영관이뭐하지' 10Byte를 초과하므로 입력 불가
          
          VARCHAR2      → 가변형 크기 
                            (상황에 따라 크기가 변경)
        
          VARCHAR2(10)  ←← '박원석'   6Byte소모        
          VARCHAR2(10)  ←←'실습조영관' 10Byte
          VARCHAR2(10)  ←←'영관이뭐하지' 10Byte를 초과하므로 입력 불가

          NCHAR         →  유니코드 기반 고정형 크기
          NCHAR(10)     ←← 10글자
          
          NVARCHAR2     →  유니코드 기반 가변형 크기 (바이트 수가 아닌 글자수가 중요함)
          NVARCHAR2(10) ←← 10글자
...
SELECT EMPNO AS "사원번호", ENAME "사원명", JOB  직종명  --""생략시 띄어쓰기 불가
FROM EMP;
...
--○ EMP 테이블에서 부서번호가 20번과 30번 직원들의 데이터 중
